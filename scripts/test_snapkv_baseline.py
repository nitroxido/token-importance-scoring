#!/usr/bin/env python
"""Test actual SnapKV baseline on LITM @ 50% budget"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test SnapKV baseline on LITM")
    parser.add_argument("--model", type=str, default="mistralai/Mistral-7B-v0.3")
    parser.add_argument("--load_in_4bit", action="store_true", default=True)
    parser.add_argument("--n_samples", type=int, default=8)
    parser.add_argument("--output", type=str, default="test_runs/snapkv_baseline_litm_50pct.csv")
    ...
    print("=" * 80)
    print("  SNAPKV BASELINE: @ 50% LITM Budget")
    print("=" * 80)
        ...
        for position in ["beginning", "middle", "end"]:
            ...
            for seed in range(args.n_samples):
                try:
                    ...
                    # Compute importance via SnapKV's method
                    importance = policy.compute_scores(attn_tensors, n_query=64, T=T)
                    
                    # Select tokens using SnapKV's strategy
                    keep_idx = policy.select_indices_to_keep(importance, T, budget, n_sink=4, n_recent=64)
