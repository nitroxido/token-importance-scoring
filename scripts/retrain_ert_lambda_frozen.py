#!/usr/bin/env python
"""
ERT Training with Frozen Lambda (Ablation Control):
  - Lambda initialized at 0.1 but NOT trainable (lr=0)
  - Importance_embedding and importance_head trainable
  - Used to isolate effect of warm-start vs actual lambda training

Configuration:
  - Batch size: 1
  - Grad accum: 4
  - Lambda: frozen at 0.1 (not in optimizer)
  - Steps: 1000
"""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="ERT with Frozen Lambda (Ablation)"
    )
    p.add_argument(
        "--base-checkpoint",
        required=True,
    ...
    p.add_argument("--lambda-init", type=float, default=0.1, help="Lambda value (frozen)")
...
def main():
    ...
    # Set lambda to frozen value
    print(f"[lambda] Freezing lambda at {args.lambda_init:.4f} (NOT trainable)", flush=True)
    model.attn_hook._lambda.data.fill_(args.lambda_init)
    model.attn_hook._lambda.requires_grad = False  # CRITICAL: Freeze it
    
    # Create optimizer for components ONLY (no lambda)
    emb_params = list(model.importance_embedding.parameters())
    head_params = list(model.importance_head.parameters())
    
    print(f"[training] Setting up single optimizer (components only):", flush=True)
    print(f"  - Importance components (emb+head) LR: {args.lr_emb:.2e}", flush=True)
    print(f"  - Lambda: frozen (not in optimizer)", flush=True)
    ...
    # Training loop
    print(f"\n[training] Starting ERT training (frozen lambda ablation)", flush=True)
    print(f"[training] Steps: {args.steps} | Grad accum: {args.grad_accum}", flush=True)
    print(f"[training] Lambda frozen at: {args.lambda_init} | Separate optimizers: disabled", flush=True)
    ...
    while global_step < args.steps:
        ...
        try:
            ...
            importance_scores[:max(1, seq_len//10)] = 70
            importance_scores[-max(1, seq_len//10):] = 70
            ...
            budget = [0.25, 0.5, 0.75][[0, 1, 2][torch.randint(3, (1,)).item()]]
            ...
            if global_step % args.grad_accum == 0:
                torch.nn.utils.clip_grad_norm_(emb_params + head_params, 1.0)
                opt_components.step()
                opt_components.zero_grad()
    ...
    print(f"[training] Final lambda: {final_lambda:+.6f} (unchanged from frozen value)", flush=True)
    ...
    metadata = {
        ...
        "training_type": "ert_frozen_lambda_ablation",
        "steps": global_step,
    ...
    print(f"\n✓ ERT training complete with frozen lambda (ablation)!")
