#!/usr/bin/env python
"""Phase B.2 Training: Real LITM Data with Token-in-Key Loss

Train importance head using real LITM data with direct token-in-key supervision.
This addresses the task alignment issue from Phase B.1.

Run: python scripts/train_phase_b_real_data.py --num_steps 1000
"""
...
def main():
    parser = argparse.ArgumentParser(description="Phase B.2 Training (Real LITM Data)")
    ...
    parser.add_argument("--output_dir", default="checkpoints/phase_b_real_data")
    ...
    model.to(device)  # Move TIS components to device
    ...
    # For training, add a simple linear scoring layer
    # This scores each token independently for the token-in-key task
    simple_scorer = torch.nn.Linear(4096, 1)  # d_model=4096
    simple_scorer.to(device)
    
    trainable_params = [
        p for p in list(simple_scorer.parameters())
        if p.requires_grad
    ]
    ...
    # Training loop
    print(f"\n[train] Starting Phase B.2 training")
    ...
    while global_step < args.num_steps:
        ...
        for batch in pbar:
            ...
            try:
                ...
                # Get importance scores using simple linear layer
                # hidden_states: [batch=1, seq_len=512, d_model=4096]
                importance_logits = simple_scorer(hidden_states).squeeze(-1)  # [batch, seq_len]
    ...
    # Save simple scorer
    scorer_state = {
        "simple_scorer": simple_scorer.state_dict(),
    }
    torch.save(scorer_state, checkpoint_dir / "tis_components.pt")
