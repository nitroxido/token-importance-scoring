#!/usr/bin/env python
"""
scripts/run_litm_paired_sweep.py

Paired position sweep: Same query-passage sets tested at 3 positions.
This isolates pure position bias by removing difficulty variance confounds.

Addresses methodological feedback:
- Tests position-only effect by holding passage content constant
- Compares paired vs independent example generation
- Validates that LITM gap persists when difficulty is controlled

Usage:
    source .venv/bin/activate
    python scripts/run_litm_paired_sweep.py --n-examples 60
"""
...
# ── Dataset loading (PAIRED SWEEP) ────────────────────────────────────────────
def load_msmarco_examples_paired(n_examples: int, k_passages: int) -> list[dict]:
    """Load MS-MARCO with PAIRED position variants (same passages, 3 positions)."""
    import datasets as hf_datasets
    print(f"[data] Loading MS-MARCO from {DATA_PATH} (PAIRED mode)...", flush=True)
    ...
    for raw in ds:
        ...
        # CRITICAL: Generate ONE base passage list, then rearrange for 3 positions
        # This isolates position-only effect (no difficulty variance)
        base_passages = list(distractors)  # distractors in fixed order
        ...
        for pos_name, gold_pos in positions:
            # Create passage list with gold at specified position
            passage_list = base_passages.copy()
            ...
            examples.append({
                ...
                "base_id":       base_count,  # shared across 3 positions
            })

        base_count += 1

    print(f"[data] Prepared {len(examples)} test cases ({base_count} base × 3 positions, PAIRED)", flush=True)
    return examples
...
def run_evaluation(args, examples, model, tokenizer, tis_head):
    """Run baseline and TIS pipelines on paired examples."""
    results = []
    ...
    print(f"\n{'='*70}\nStarting paired position sweep: {len(examples)} examples × {len(pipelines)} pipelines\n{'='*70}\n", flush=True)
    ...
    for pipeline_name, pipeline_desc in pipelines:
        ...
        for ex in examples:
            ...
            if pipeline_name == "baseline":
                ...
                ordered_passages, ranked_indices = reorder_by_tis(passages, model, tokenizer, tis_head)
...
def save_results(summary_rows, metadata, args):
    """Save summary CSV and metadata JSON."""
    os.makedirs(RESULTS_DIR, exist_ok=True)
    
    # Summary CSV
    summary_path = os.path.join(RESULTS_DIR, "litm_paired_summary.csv")
    with open(summary_path, "w", newline="") as f:
        fieldnames = ["pipeline", "position", "n", "em", "f1"]
    ...
    # Metadata JSON
    metadata_path = os.path.join(RESULTS_DIR, "litm_paired_metadata.json")
    with open(metadata_path, "w") as f:
        json.dump(metadata, f, indent=2)
    print(f"[save] {metadata_path}", flush=True)
    
    # Print summary table
    print(f"\n{'='*70}\nPaired Position Sweep Summary\n{'='*70}\n")
    for pipeline in ["baseline", "tis"]:
        subset = [r for r in summary_rows if r["pipeline"] == pipeline]
        if not subset:
            continue
        print(f"{pipeline.upper()}:")
        print(f"  Position | EM    | F1")
        print(f"  ---------|-------|-------")
        for row in subset:
            print(f"  {row['position']:8s} | {row['em']:.3f} | {row['f1']:.3f}")
        ...
        print(f"  Average  | {avg_em:.3f} |")
        print(f"  LITM gap | {litm_gap:.3f} |\n")
...
def main():
    ...
    # Metadata
    metadata = {
        "experiment_id": "litm_paired_position_sweep",
        ...
        "dataset": {
            ...
            "num_positions": 3,
            "examples_are_paired": True,
            "position_isolation": "Same passage sets tested at 3 positions (pure position effect)",
    ...
    # Analyze
    summary_rows = generate_summaries(results, args)
    
    # Save
    save_results(summary_rows, metadata, args)
    
    print(f"{'='*70}\n✓ Paired position sweep complete!\n{'='*70}\n", flush=True)
